  ...
  useLayoutEffect(() => {
    navigation.setOptions({
      ...
      headerRight: () => {
        return (
          <View>
            <Text style={styles.textTop}>Project List</Text>
          </View>
        );
      },
    });
          ...
          <View key={project.id} style={styles.projectContainer}>
            ...
            {project.tasks.map((task) => (
              <View key={task.id} style={styles.taskContainer}>
                ...
                <Text style={styles.taskAssignedTo}>
                  Assigned to: {getMemberName(task.assignedTo)}
                </Text>
                <Text style={styles.taskDueDate}>
                  Due Date: {task.dueDate.toDate().toDateString()}
                </Text>
                <Text style={styles.taskStatus}>Status: {task.status}</Text>
                <CheckBox
                  value={task.status === "completed"}
                  onValueChange={() =>
                    handleTaskCompletion(project.id, task.id, task.status)
                  }
                />
              </View>
            ))}
